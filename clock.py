...
class Clock():
    ...
    def zmanim_list(self, zman_text):
        zmanim_list = [self.font.render(f'{zman.split(" (")[0]} - {translations.get(zman, zman).strip()} - {time.strip()}', True, self.TEXT_COLOR, self.BG_COLOR) for zman, time in zman_text.items()]
